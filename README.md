...
процесс **docker-proxy**, который слушает сетевой tcp порт 9292
 ...
 - Был отредактирован файл **docker-compose.yml** под кейс с множеством сетей, сетевых алиасов. Также были параметризированы с помощью переменных окружения порт публикации сервиса ui, версии сервисов, имя пользователя. Все переменные были описаные в файле **variables.env**. Сам файл был добавлен в **.gitignore**. Для примера был создан файл **variables.env.example**
 ...
 - В задании со * был рассмотрен вопрос образования имени проекта. При создании контейнеров название контейнера начинается с названия с названия проекта. По умолчанию название проекта соответствует названию базовой директории проекта. Для смены названия проекта можно воспользоваться либо параметром **-p** либо с помощью переменной **COMPOSE_PROJECT_NAME**.
